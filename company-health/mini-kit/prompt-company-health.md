...
4. Update or remove the company's `watch_note` if the situation changed.
5. Run: `python3 mini-kit/score_engine.py register.json --event "<TICKER> <quarter> report" --date <today>` (from the company-health dir). Never hand-edit totals/subscores — the engine owns arithmetic.
6. Rebuild the dashboard: `python3 mini-kit/build_dashboard.py .`
7. Append one line per ticker to `log.md` under `## Log` (newest at bottom), format:
